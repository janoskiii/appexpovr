...
<div>
  ...
  <tbody>
      <td> @foreach($nacionalidad as $nacionalidad)
    <tr>
  ...
  <td>{{$nacionalidad->nacionalidad}}</td>
    ...
    <td>
      <button type="button" class="btn btn-warning btn-editar" data-id="{{$nacionalidad->id}}" data-pais="{{$nacionalidad->pais}}" data-nacionalidad="{{$nacionalidad->nacionalidad}}" data-toggle="modal" data-target="#exampleModaleditar">
        Editar
      </button><!--<a href="{{url('/nacionalidad/edit',$nacionalidad->id)}}" ><span class="btn btn-warning "data-toggle="modal" data-target="#exampleModal"> editar</span></a>-->
<a href="{{ url('/nacionalidad/destroy',$nacionalidad->id)}}"><span class="btn btn-danger">Eliminar</span></a>
</td>
...
<p>
  <input type="text" name="nacion" placeholder="Sede" class="form-control" required>
</p>
<p>
...
<h4>editar nacionalidad</h4>
</div>

<div class="panel-body">
  <form action="{{route('editarNacionalidad')}}" method="post">
{!! csrf_field() !!}
<p>
  <input id="campoEditarNombre" type="text" name="nombre" placeholder="Nombre" class="form-control" required >
</p>
<p>
  <input id="campoEditarNacion" type="text" name="nacion" placeholder="Sede" class="form-control" required>
</p>
<p>
  <input type="submit" value="Guardar" class="btn btn-success">
  <button type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button>
...
@endif
    ...
    <script type="text/javascript">
      $(document).on("click", ".btn-editar", function(){
        ...
        $("#campoEditarNombre").val(pais);
        $("#campoEditarNacion").val(nacionalidad);
      });
    </script>
  </body>
</html>
